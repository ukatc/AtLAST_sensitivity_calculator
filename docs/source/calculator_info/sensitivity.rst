...
The system temperature is calculated differently depending on which instrument is selected for the calculation. Examples of existing instruments for other telescopes have been included in the sensitivity calculator. Specifically, we have initially included examples of heterodyne spectroscopic instruments (:doc:`CHAI <instruments/chai_tsys>`, :doc:`FINER <instruments/finer_tsys>`, and :doc:`SEPIA <instruments/sepia_tsys>`), a LEKID based continuum instrument (:doc:`MUSCAT <instruments/muscat_tsys>`) and a MKID based IFU (:doc:`TIFUUN <instruments/tifuun_tsys>`). There is also the capability to add further instruments to this, as described in :doc:`adding a new instrument <../developer_guide/adding_new_instrument>`. The calculator automatically selects the instrument based on the requested observing frequency and channel bandwidth as described in the :doc:`instrument overview <instrument_overview>`.
